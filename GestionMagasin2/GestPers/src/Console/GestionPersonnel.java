...
public class GestionPersonnel {
   ...
   // Méthode pour charger les données du personnel depuis un fichier XML
   ...
   // Méthode pour modifier les données d'une personne dans la liste du personnel
   protected void ModifPersonnel(ArrayList<Personnel> Person) {
      ...
      for(int count = 0; Person.size() > count; ++count) {
         if (this.nom.equalsIgnoreCase(((Personnel)Person.get(count)).getNom().trim())) {
            ...
            Personnel var10000 = (Personnel)Person.get(count);
            WordUtils var10001 = this.wordutils;
            var10000.setNom(WordUtils.mettreEnMajscule(InputData.inputNomPrenom("nom", this.tailleNom)));
            var10000 = (Personnel)Person.get(count);
            var10001 = this.wordutils;
            var10000.setPrenom(WordUtils.mettreEnMajscule(InputData.inputNomPrenom("prenom", this.tailleNom)));
            var10000 = (Personnel)Person.get(count);
            var10001 = this.wordutils;
            var10000.setEmail(WordUtils.mettreEnMinusculeMail(InputData.inputEmail()));
            egal = true;
         }
      }
   ...
   // Méthode pour supprimer une personne de la liste du personnel
   protected void SuppressionPersonnel(ArrayList<Personnel> Person) {
      ...
      while(var3.hasNext()) {
         ...
         if (personnel.getNom().contains(this.nom)) {
            ...
            InputData var10000 = this.inputData;
   ...
   // Méthode pour ajouter une nouvelle personne au personnel
